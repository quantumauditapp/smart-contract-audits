...
|----------|--------|-----------|-------|
| **Technical** | 7/10 | Low | The contract leverages battle-tested OpenZeppelin libraries for its ERC-20 and access control functionalities (7.2 Code Security). The `_swapAndLiquify` mechanism includes a reentrancy guard (`inSwapA |
| **Governance / Economics** | 7/10 | Low | The contract exhibits a high degree of centralization, with the owner possessing extensive control over critical economic parameters (7.5 Governance). The owner can unilaterally adjust all fee percent |
| **Upgrades** | 9/10 | Low | The contract is not designed with an upgrade mechanism (7.7 Upgrades). This means its logic is immutable once deployed, providing certainty regarding its behavior. However, any discovered vulnerabilit |
